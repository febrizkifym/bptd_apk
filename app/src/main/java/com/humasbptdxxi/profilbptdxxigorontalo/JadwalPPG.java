...
public class JadwalPPG extends AppCompatActivity {
    Toolbar toolbar;
    ...
    final List<String> namaKapal = new ArrayList<String>();
    final List<String> keterangan = new ArrayList<String>();
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET, url, response -> {
            ...
            try {
                ...
                for (int i = 0; i < objArray.length(); i++) {
                    ...
                    if (j.getString("id_kapal").equals("1")) {
                        Event titik = new Event(Color.RED, tanggal, keterangan);
                        jadwalCalendar.addEvent(titik);
                    } else if (j.getString("id_kapal").equals("2")) {
                        Event titik = new Event(Color.YELLOW, tanggal, keterangan);
                        jadwalCalendar.addEvent(titik);
                    }
                ...
                if(!jadwalCalendar.getEvents(dateClicked).isEmpty()){
                    ...
                    for(int i=0;i<jadwalCalendar.getEvents(dateClicked).size();i++){
                        ...
                        if(events.get(i).getColor() == Color.RED){
                            namaKapal.add("KMP. MOINIT (Gorontalo-Pagimana)");
                        }else if(events.get(i).getColor() == Color.YELLOW){
                            namaKapal.add("KMP. TUNA TOMINI (Gorontalo-Wakai)");
                        }
